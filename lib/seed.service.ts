...
import { More, SeederModuleOptions, SeederOptions } from ".";
...
export class SeederService {
  ...
  private data: any;
  constructor(
    @Inject(seeder_token.options) public options: Partial<
  ...
  private async createItem(
    ...
    if (
      typeof enableAutoId === "boolean" && enableAutoId &&
      typeof autoId === "number"
    ) {
      item[autoIdFieldName] = autoId;
    }
    // Called everyone function if exist !
    if (!disableEveryOne && typeof (this.seed?.everyone) === "function") {
